...
function StaffDashboard() {
  ...
  const staffNotices = notices.filter(n => n.audience === 'staff' || n.audience === 'all');

  const filteredNotices = staffNotices.filter(n =>
  ...
  const renderBadge = (audience) => (
    <span style={{
      backgroundColor: audience === 'staff' ? '##eeeeee' : '#007bff',
      color: '#fff',
  ...
  return (
    <div className="container py-4" style={{ minHeight: '100vh', background: 'linear-gradient(to right, #fdfbfb, #ebedee)' }}>
      <div className="d-flex justify-content-between align-items-center mb-4">
        <h2 className="fw-bold text-info">👨‍🏫 Staff Dashboard</h2>
        <LogoutButton />
      </div>
      ...
      {currentNotices.map((n, i) => (
        <div key={i} className="card shadow-sm mb-3 border border-light">
          <div className={`card-header text-white d-flex justify-content-between align-items-center ${n.audience === 'staff' ? 'bg-info' : 'bg-primary'}`}>
      ...
      {totalPages > 1 && (
        <div className="text-center my-4">
          {[...Array(totalPages)].map((_, i) => (
            <button
              key={i}
              className={`btn btn-sm mx-1 ${currentPage === i + 1 ? 'btn-info' : 'btn-outline-info'}`}
